...
i = 42;
    ...
    function [lossv] = predictors()
        for j=1:1000
            ...
            losst = validateKNN(test.lying,test.sitting,test.standing,...
             test.walking,test.turnR,test.turnL, kNNModel);        
            lossv(j) = 1 - losst;
        end
    end
    ...
    function model = trainPlotKNN(varargin) 
        ...
        %t = templateSVM('KernelFunction', 'gaussian','KernelScale',1,'BoxConstraint',41.356,'Standardize',1);
        %md = fitcecoc(trainingTable,labels,'Learners',t,'ClassNames',{'Sedentary' 'Moderate' 'Rigorous' }); 
        md = fitcsvm(trainingTable,labels,'KernelFunction','gaussian','KernelScale',1,'BoxConstraint',41.356,'Standardize',1);
        ...
        svs = bsxfun(@plus,bsxfun(@times,svs,md.Sigma),md.Mu);
        numberSV = length(svs);
        ...
        for i=1:numberSV
            ...
            for j=1:length(x)
                for k=1:length(y)
                    t = [x(j);y(k)];
                    f(j,k) = f(j,k) + alpha*sign*kernel(scale, t, sv);
                end
            end    
        end
        ...
        shading flat;        
        alpha(.6);
        contourf(x1Grid,x2Grid, f, 1);
        
        %x = repmat(1/scale,size(x,1),1).*(x+repmat(shift,size(x,1),1));
        %y = repmat(1/scale,size(y,1),1).*(y+repmat(shift,size(y,1),1));
        
        
        %{
        figure;
        hold on;
        h = zeros(2,1); % Preallocate for handles        
        ...
        for n=1:2
            ...
            plot(x,y,markers{n},'Color',colorsR{n});  
            xlabel(table.Properties.VariableNames{1});    
            ylabel(table.Properties.VariableNames{2});
        ...
        contour(x1Grid,x2Grid,reshape(scores(:,1),size(x1Grid)),[0 0],'Color',[0 0 0],'LineWidth',1.5);    
        %contour(x1Grid,x2Grid,reshape(scores(:,2),size(x1Grid)),[0 0],'Color',[0 0.8 0.8],'LineWidth',1.5);       
        %contour(x1Grid,x2Grid,reshape(scores(:,3),size(x1Grid)),[0 0],'Color',[0.8 0.8 0],'LineWidth',1.5); 
        
        %title 'IMA -- ECOC Support Vectors';
        predictorNames = md.PredictorNames;
        classNames = {'Sedentary' 'Moderate'};
        xlabel(predictorNames{1});
        ylabel(predictorNames{2});        
        legend([classNames,{'Decision Boundary'}],...
            'Location','Best');
        hold off
        grid on;      
        %}
