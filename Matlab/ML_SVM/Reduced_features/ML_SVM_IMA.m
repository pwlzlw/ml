...
i = 42;
    ...
    function losst = validateKNN(varargin)
        ...
        %trainingTable(:,[1,3:9,11:13,15:18]) = [];
    ...
    function model = trainPlotKNN(varargin) 
        ...
        trainingTable(:,[1,3:9,11:13,15:18]) = [];   
        t = templateSVM('KernelFunction', 'gaussian','KernelScale',1.5727,'BoxConstraint',41.356,'Standardize',1);
        md = fitcecoc(trainingTable,labels,'Learners',t,'ClassNames',{'Sedentary' 'Moderate' 'Rigorous' });  
        length(md.BinaryLearners{1}.SupportVectors)+length(md.BinaryLearners{2}.SupportVectors)+length(md.BinaryLearners{3}.SupportVectors)
        model = md;
    end    
    %feature extraction
