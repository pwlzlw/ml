...
i = 42;
    ...
    function model = trainPlotKNN(varargin) 
        ...
        L = size(md.CodingMatrix,2); % Number of SVMs
        sv = cell(L,1); % Preallocate for support vector indices
        for j = 1:L;
            SVM = md.BinaryLearners{j};
        ...
        for n=1:3
            table = tbl{n};
            plot(table{:,1},table{:,2},markers{n},'Color',colorsR{n});  
        ...
        d = 1;
        [x1Grid,x2Grid] = meshgrid(min(trainingTable{:,1}):d:max(trainingTable{:,1}),...
        min(trainingTable{:,2}):d:max(trainingTable{:,2}));
        xGrid = [x1Grid(:),x2Grid(:)];
        ...
        classNames = {'Sedentary' 'Moderate' 'Rigorous' };
        xlabel(predictorNames{1});
        ylabel(predictorNames{2});        
        legend([classNames,{'Boundary - SVM 1',...
            'Boundary - SVM 2','Boundary - SVM 3'}],...
            'Location','Best');
        hold off
        grid on;        
        model = md;
