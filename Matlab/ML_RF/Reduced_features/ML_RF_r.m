...
mag = load('dataMAGintMK3.mat');

Features = combineFeatures (FeaturesACC,FeaturesIMA);
Features = combineFeatures (FeaturesACC,mag,FeaturesIMA);
...
i = 42;
    ...
    function [lossv] = predictors()
        for j=1:10000
            ...
            kNNModel = trainPlotKNN(train.running,train.jumping,train.exercising);        
            losst = validateKNN(test.running,test.jumping,test.exercising, kNNModel);        
            lossv(j) = 1 - losst;
        end
    end
    ...
    function model = trainPlotKNN(varargin) 
        ...
        md = TreeBagger(10,trainingTable,labels,'method','classification','OOBPrediction','on','MinLeafSize',74,...
        'MaxNumSplits',5);      
        ...
        ylabel 'Out-of-bag classification error [no units]';
        %}
        model = md;
    end    
    %feature extraction
