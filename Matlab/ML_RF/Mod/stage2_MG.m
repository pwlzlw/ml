...
cd('figures');
cd('Final_Figures');
Features = combineFeatures (FeaturesACC, gyroPCA, FeaturesIMA);
...
%IMAPLOT(Features.walking,Features.turnR,Features.turnL);
%}
...
validateRF(test.walking,test.turnR,test.turnL, rfmodel);
...
%use multiple binary in one 
    ...
    function validateRF(varargin)
        ...
        %trainingTable(:,[7:9,11:13,17]) = []; %muy muz all va, max,miny,mex,ima. mez
        trainingTable(:,[1,4:14,16:17]) = []; 
        trainingTable(:,[6:22]) = []; 
        ...
        confMat = confusionmat(labels,oofLabel,'order',{'Walking' 'TurnL' 'TurnR'});   
    ...
    function model = trainPlotRF(varargin)
        ...
        %trainingTable(:,[7:9,11:13,17]) = []; %all mu all va, max,miny,mex,ima. mez
        %trainingTable(:,[13:29]) = [];
        ...
        'OOBPredictorImportance','on','MinLeafSize',71,'MaxNumSplits',5);      
        ...
        plotconfusion(YMat,oofLabelMat);
        h = gca;
        title('Confusion matrix  - moderate with gyroscope');
        h.XTickLabel = [isLabels; {''}];
        h.YTickLabel = [isLabels; {''}];       
        set(findobj(gcf,'facecolor',[120,230,180]./255),'facecolor',royalg);
        set(findobj(gcf,'facecolor',[230,140,140]./255),'facecolor',royalr);
    ...
    function k = IMAPLOT(varargin)
        ...
        tblr = table;
        tbl = {};
        tbl{1} = tblr(strcmp(tblr{:,end}, 'Walking' ),:);
        tbl{2} = tblr(strcmp(tblr{:,end}, 'TurnL' ),:);
        tbl{3} = tblr(strcmp(tblr{:,end}, 'TurnR' ),:);           
        %pcacandidates = [6,9,11,13,14,17];
        for j=1:noColumns 
            figure(j);
            hold on;
            ...
            for n=1:3
                table = tbl{n};
                plot(table{:,z},table{:,j},markers{n});  
