...
validateRF(test.lying,test.sitting,test.standing,...
...
%use multiple binary in one 
    ...
    function validateRF(varargin)
       ...
       % trainingTable(:,[4:6,11:13,15,17:18]) = []; %all va, max,miny,mex,ima
        trainingTable(:,[1,3:9,11:13,15:16,18]) = [];
        ...
        confMat = confusionmat(labels,oofLabel,'order',{'Sedentary' 'Moderate' 'Rigorous'});   
        ...
        cd('figures');
        cd('Final_Figures');
        %print('cmIMA','-dpng','-r300');
        %saveas(gcf,'cmIMA.png','-dpng','-r0')
    end    
    ...
    function model = trainPlotRF(varargin)
        ...
        %trainingTable(:,[4:6,11:13,15,17:18]) = []; %all va, max,miny,mex,ima
        %trainingTable(:,[7:9,11:13,15,17:18]) = []; %all va, max,miny,mex,ima
        trainingTable(:,[1,3:9,11:13,15:18]) = []; %muy,max,miy,IMA

        labels = trainingTable(:,{'Label_P'});
        trainingTable(:,{'Label','Index'}) = [];  
        
        md = TreeBagger(20,trainingTable,'Label_P','Method','classification',...
        'OOBPredictorImportance','on','MaxNumSplits',5,'MinLeafSize',71,'NumPredictorsToSample',2);  
        cd('..')    
        cd('..')
        cd('_RF_proper')    
        cd('IMA')
        ...
        oobErrorBaggedEnsemble = oobError(md);%, 'Mode', 'ensemble');
        figure(1);
        ...
        plotconfusion(YMat,oofLabelMat);
        h = gca;
        title('Confusion matrix  - stage 1');
        h.XTickLabel = [isLabels; {''}];
        h.YTickLabel = [isLabels; {''}];       
        set(findobj(gcf,'facecolor',[120,230,180]./255),'facecolor',royalg);
        set(findobj(gcf,'facecolor',[230,140,140]./255),'facecolor',royalr);
        set(findobj(gcf,'facecolor',[0.5,0.5,0.5]),'facecolor',[1 1 1]);  
        %print('IMAmofo','-dpng','-r300');
        model = md;
    end    
