...
standing2([1:227,2532:2848],:) = [];
standing3([1:227,2532:3115],:) = [];
standing4([1:227,2532:2766],:) = [];
...
save(['dataMAgint_mag.mat'],'sitting','lying','standing','running','jumping','exercising','walking','turnR','turnL');
...
%feature extraction
    function features = extractFeatures(rawData,window,label_1)
        ...
        for i=1:n
            ...
            accX = rawData{:,2}./(0.15*10^(-6)); 
        accY = rawData{:,3}./(0.15*10^(-6));
        accZ = rawData{:,4}./(0.15*10^(-6));
        
        magi{i} = getIMA(accX(indexStart:indexEnd), accY(indexStart:indexEnd)...
        ...
        magi = cellfun(@(v)v(1),magi);

        features = table(magi', labels, index, labels_primary);
        ...
        features.Properties.VariableNames = {'MGM','Label','Index','Label_P'};;
    end
    ...
    function IMA = getIMA(x,y,z)
        ...
        sum = 0;
        for i=1:w_length 
            sum = sum + abs( sqrt( x(i)^2 + y(i)^2 + z(i)^2 ));
        end
        IMA = sum/w_length;        
    end
