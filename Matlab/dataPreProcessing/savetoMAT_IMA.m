...
%plot raw data
%plotRawData(jumping2,1,'[ g ]',royalb);
%plotRawData(walking2,2,'[ g ]',royalg);
%plotRawData(standing2,3,'[ g ]',royalr);
%plotRawData(exercise7,4,'[ g ]',[0 0 0]);
...
save(['dataIMA.mat'],'sitting','lying','standing','running','jumping','exercising','walking','turnR','turnL');
...
%feature extraction
    function features = extractFeatures(rawData,window,label_1)
        ...
        for i=1:n
            ...
            %IMA per window
            accX = rawData{:,2}.*2048./8; 
            accY = rawData{:,3}.*2048./8;
            accZ = rawData{:,4}.*2048./8;
            ima{i} = getIMA(accX(indexStart:indexEnd), accY(indexStart:indexEnd)...
                , accZ(indexStart:indexEnd));       
        end
        len = length( mu );
        
        %labels
        labels = repmat(label(1),len,1);
        labels_primary = repmat({label_1},len,1);
        if  strcmp({label_1},'Sedentary')
            index = repmat(0,len,1);
        elseif strcmp({label_1},'Rigorous')
            index = repmat(1,len,1);
        else
            index = repmat(0.5,len,1);            
        end         
        %combined features
        ima = cellfun(@(v)v(1),ima);
        %ima = normalizeFeature(ima);
        ...
        %featureV = vertcat(muX,muY,muZ,siX,siY,siZ,vaX,vaY,vaZ,maX,maY,maZ,miX,miY,miZ,meX,meY,meZ);
        features = table(ima', labels, index, labels_primary);    
        %features = table(muX',muY',muZ',labels); 
        features.Properties.VariableNames = {'IMA','Label','Index','Label_P'};
        %features.Properties.VariableNames = {'muX','muY','muZ','Label'};
    end
