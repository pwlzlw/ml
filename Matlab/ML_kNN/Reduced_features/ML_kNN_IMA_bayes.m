...
i = 42;
    ...
    function model = trainPlotKNN(varargin) 
        ...
        num = optimizableVariable('k',[1,43],'Type','integer');
        dst = optimizableVariable('metric',{'chebychev','euclidean','minkowski'});
        c = cvpartition(9009,'Kfold',50);
        
        fun = @(x)kfoldLoss(fitcknn(trainingTable,labels,'CVPartition',c,'NumNeighbors',x.k,...
                    'Distance',char(x.metric),'NSMethod','exhaustive'));
        ...
        w.MarkerFaceColor = 'r';
        hold off        
        legend([h1(3) h w],{'Observed points','Model mean','Model minimum feasable'},'Location','northwest');
        cd('..')
        cd('epstopdf')
        print -depsc2 knnbayes.eps
