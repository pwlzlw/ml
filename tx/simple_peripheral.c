/******************************************************************************
 ...
 @brief This is the transmitter firmware based on simple peripheral project supplied by TI.

 Group: CMCU, SCS
 Target Device: CC2640R2

 ******************************************************************************
 * Author : Pawel Zalewski (edit of the SDK file)
 * Wrote code for downloading and sending the samples and the sensor's interrupt routines
 * and the classifier routines.
 * Original provided by Texas Instruments as a part of their SDK, included the sensor
 * integration and all associated functions with this.
 * The profile UUID is 0xFFE0
 * INCLUDES
 */
#include <string.h>
...
// Advertising interval when devicbe is discoverable (units of 625us, 160=100ms)
#define DEFAULT_ADVERTISING_INTERVAL          1730/2
...
#define DISPLAY_PERIODIC_EVT_PERIOD           2000

// Application specific event ID for HCI Connection Event End Events
#define SBP_HCI_CONN_EVT_END_EVT              0x0001

#define ADV_UPDATE_BUFFER_PERIOD              2300
#define FEATURE_UPDATE_PERIOD                 1000
...
#define SBP_TASK_STACK_SIZE                   1932//1288//644
#endif

// Application events - 32 possible bits assigned for this
// NB these now need to be registered via SBP_ALL_EVENTS  !!!

#define SBP_STATE_CHANGE_EVT                  0x0001
#define SBP_CHAR_CHANGE_EVT                   0x0002
#define SBP_KEY_CHANGE_EVT                    0x0004

#define DO_MC3672_EVT                         0x0008
#define DO_MC6470_MAG_EVT                     0x0020
//#define DO_MC6470_ACC_EVT                     0x0020
#define DO_ICM20948_EVT                       0x0010
//#define DO_DISPLAY_EVT                        0X0080
#define DO_ADV_UPDATE_EVT                     0x0040
//#define DO_SCIF_ALERT_EVT                     0x0200
//#define DO_SCIF_READY_EVT                     0x0400
#define DO_FEATURES_EVT                            0x0080
...
static bool ic_MC6470 = 0, ic_MC3672 = 0, ic_ICM20948;
static activity_t tree1, tree2;
//FIFO registers
static uint8_t R[3][MAX_SAMPLES] = {0};//, R2[MAX_SAMPLES], R3[MAX_SAMPLES];
static uint8_t RG[3][MAX_SAMPLES] = {0};//, R2G[MAX_SAMPLES], R3G[MAX_SAMPLES];

static float featureVector[FEATURE_NUMBER] = {0};
...
static Clock_Struct MC3672_eventClock;
static Clock_Struct ICM20948_eventClock;
static Clock_Struct MC6470_ACC_eventClock;
static Clock_Struct MC6470_MAG_eventClock;
static Clock_Struct OEL_DISPLAY_eventClock;
static Clock_Struct SCIF_ALERT_eventClock;
static Clock_Struct SCIF_READY_eventClock;
static Clock_Struct ADV_update_buffer_Clock;
static Clock_Struct FEATURES_eventClock;

// Queue object used for app messages
static Queue_Struct appMsg;
static Queue_Handle appMsgQueue;

//static Semaphore_Struct Init_done_semaphore;
//static Semaphore_Handle semapHandle;
...
void Update_ADV_buffer_data(void);
void Compute_features(uint8_t );
...
void static Process_MC3672_data(uint8_t);


void static Process_ICM20948_data(uint8_t);
...
static void SimpleBLEPeripheral_init(void)
{
  ...
  Util_constructClock(&MC3672_eventClock, SimpleBLEPeripheral_clockHandler, 0, 0, false, DO_MC3672_EVT);

  Util_constructClock(&FEATURES_eventClock, SimpleBLEPeripheral_clockHandler, 0, 0 , false, DO_FEATURES_EVT);

  Util_constructClock(&ICM20948_eventClock, SimpleBLEPeripheral_clockHandler, 0, 0, false, DO_ICM20948_EVT);

  Util_constructClock(&MC6470_MAG_eventClock, SimpleBLEPeripheral_clockHandler, 0, 0, false, DO_MC6470_MAG_EVT);

  Util_constructClock(&ADV_update_buffer_Clock, SimpleBLEPeripheral_clockHandler, FEATURE_UPDATE_PERIOD , 0, false, DO_FEATURES_EVT);
...
//  / HCI_LE_SetDefaultPhyCmd(LL_PHY_USE_ANY_PHY, LL_PHY_1_MBPS | LL_PHY_2_MBPS| HCI_PHY_CODED, LL_PHY_1_MBPS | LL_PHY_2_MBPS| HCI_PHY_CODED);
 // HCI_LE_SetDefaultPhyCmd(LL_PHY_2_MBPS,LL_PHY_2_MBPS, LL_PHY_2_MBPS);

  // Start the GAPRole
  VOID GAPRole_StartDevice(&SimpleBLEPeripheral_gapRoleCBs);

  Display_printf(dispHandle, 0, 0, "BLE Peripheral");
...
static void SimpleBLEPeripheral_taskFxn(UArg a0, UArg a1)
{
  ...
  SimpleBLEPeripheral_init();
    ...
    ic_ICM20948 = ICM20948_start();
    ic_MC3672 = MC3672_start();

    ICM20948_enable_FIFO();
    MC3672_enableFIFO();
    ICM20948_sleep(1);

    if (!ic_MC3672 ) {
        System_printf("Unsuccessful MC3672 init");
    }

    if (!ic_MC6470 ) {
        System_printf("Unsuccessful MC6470 init");
    }

    if (!ic_ICM20948 ) {
        Display_printf(dispHandle, 2, 0, "No gyro.");
    }

    Timestamp_getFreq(&frequency);
    freq = frequency.hi;

  Util_startClock(&ADV_update_buffer_Clock);
  //Util_startClock(&FEATURES_eventClock);
  ...
  for (;;)
  {
    ...
    if (events)
    {
      ...
      //check for update buffer evt
      if (events & DO_ADV_UPDATE_EVT)
      {
        events &= ~DO_ADV_UPDATE_EVT;
        Update_ADV_buffer_data();
      }
      //check for event from Accelerometer MC3672 conversion
      if (events & DO_MC3672_EVT)
      {
        events &= ~DO_MC3672_EVT;
        Process_MC3672_data(1);
      }
      //compute features
    if (events & DO_FEATURES_EVT)
    {
      events &= ~DO_FEATURES_EVT;
      Compute_features(1);
    }
      //check for event from Magnetometer MC3672 conversion
      if (events & DO_MC6470_MAG_EVT)
      {
        events &= ~DO_MC6470_MAG_EVT;
        //Process_MC6470_MAG_data();
      }
      //check for event from the gyroscope
      if (events & DO_ICM20948_EVT)
      {
        events &= ~DO_ICM20948_EVT;
        Process_ICM20948_data(1);
      }
...
/* @fn Process_MC3672_data - copy the data from the sensor into advert array, clear interrupt*/
static void Process_MC3672_data( uint8_t a ) {
  uint8_t SPI_Accel0_in_buffer[MAX_SAMPLES];
  SPI_Accel0_in_buffer[0] = a;
  //copy the previous FIFO output to R1
  memcpy(R[0],R[2],MAX_SAMPLES);
  MC3672_processFIFO(SPI_Accel0_in_buffer);
  MC3672_clrIRQ();
  //get the current FIFO to R2
  memcpy(R[2],SPI_Accel0_in_buffer,MAX_SAMPLES);
  //compute the overlapping window in R3
  memcpy(R[1],R[0]+MAX_S_HALF,MAX_S_HALF);
  memcpy(R[1]+MAX_S_HALF,R[2],MAX_S_HALF);
  //dispatch update feature task
  Util_startClock(&FEATURES_eventClock);
}
float gMUX;
/* @fn Process_ICM20948_data - copy the data from the gyro into advert array, clear interrupt */
static void Process_ICM20948_data( uint8_t a ) {
  uint8_t SPI_Gyr0_in_buffer[MAX_SAMPLES] = {0};
  SPI_Gyr0_in_buffer[0] = a;
  memcpy(RG[0],RG[2],MAX_SAMPLES);
  IMC20948_process_FIFO(SPI_Gyr0_in_buffer);
  memcpy(RG[2],SPI_Gyr0_in_buffer,MAX_SAMPLES);
  memcpy(RG[1],RG[0]+MAX_S_HALF,MAX_S_HALF);
  memcpy(RG[1]+MAX_S_HALF,RG[2],MAX_S_HALF);
  ICM20948_clr_irq();
}

/* @fn Process_MC6479_MAG_data - copy the data from the sensor into advert array, clear interrupt
static void Process_MC6470_MAG_data (void) {
  uint8_t I2C_Mag0_in_buffer[6];
  MC6470_MAG_readMgntmtr(I2C_Mag0_in_buffer);
  //memcpy(dataBuffer+12,I2C_Mag0_in_buffer,6);
}*/
/* @fn Update_ADV_buffer_data - updates the advert data array*/
void Update_ADV_buffer_data(void) {
 //notting
}
/* @fn Compute_features - computes the features for two sliding windows
 * Format per window :
 * Feature | #Index
 * IMA     |  0,1
 *
...
static uint8_t regNo = 0, regMem =0;

static uint8_t gyroFlag = 0;
void Compute_features(uint8_t a) {
    uint8_t val;
    //regNo = 2;
    //regMem = 2;
    ...
    computeFV(R[regNo],featureVector,0);

    stage1_RF(featureVector, ret);
    tree1 = evalS1(ret[0],ret[1],ret[2]);

    if (tree1 == Mod) {
        //Display_printf(dispHandle, 1, 0, "Moderate activity level.");
        //enter stage 2 tree
        computeFV(R[regNo],featureVector,2);
        if (gyroFlag == 1) {
            //evaluate the tree model with gyro features
            int16_t gyroXYZ[96];
            intoInteger16(RG[regMem] ,gyroXYZ, 1);
            gMUX = getMU(gyroXYZ,0); //muX
            featureVector[12] = gMUX;
            stage2_MG(featureVector, ret2);
            tree2 = evalMG(ret2[0],ret2[1],ret2[2]);
            regMem++;
            regMem &= 0x01;
        } else {
            //try your best with accelerometer only, we still have 70% chance of being correct
            stage2_MA(featureVector,ret2);
            tree2 = evalMA(ret2[0],ret2[1],ret2[2]);
            regMem = 0;
        }
        gyroFlag = 1;
    } else if (tree1 == Rig) {
        gyroFlag = 0;
        //enter stage 2 tree
        computeFV(R[regNo],featureVector,3);
        stage2_R(featureVector,ret2);
        tree2 = evalR(ret2[0],ret2[1],ret2[2]);
        //Display_printf(dispHandle, 1, 0, "Rigorous activity level.");
    } else {
        //enter stage 2 tree
        gyroFlag = 0;
        computeFV(R[regNo],featureVector,1);
        stage2_S(featureVector,ret2);
        tree2 = evalS2(ret2[0],ret2[1],ret2[2]);
        //Display_printf(dispHandle, 1, 0, "Sedentary activity level.");
    }
    //wakeup gyro for the next window, as it is sampled at 17Hz there will surely be samples
    // when we enter here the next time.

    if(gyroFlag == 1) {
        ICM20948_sleep(0);
    } else {
        //empty all gyro registers...
        uint8_t k,m;
        for(k = 0;k<3;k++) {
            for (m= 0;m<96;m++) {
                RG[k][m] = 0;
    ...
    //first two msb are s/m/r ima based activity
    //5:4 are sedentary detail
    //3:2 are moderate detail
    //1:0 are rigorous detail
    act = tree1 | tree2;
    uint8_t d,b,c;
    d = ret2[0]>>8;
    b = ret2[1]>>8;
    c = ret2[2]>>8;
    memcpy(advertData+7,&act,1);
    memcpy(advertData+8,&d,1);
    memcpy(advertData+9,&b,1);
    memcpy(advertData+10,&c,1);
    GAPRole_SetParameter(GAPROLE_ADVERT_DATA, sizeof(advertData), advertData);
    regNo++;
    regNo &= 0x01;
    //t1 = Timestamp_get32();
...
/*****************************************************************************
* @fn MC3672_IRQ_Handler - interrupt handler, called from the sesnor driver*/
void  MC3672_IRQ_Handler(PIN_Handle handle, PIN_Id pinId) {
  //if(in_advertising_mode == true) {
      Util_startClock(&MC3672_eventClock);
  //}
}
/* @fn ICM20948_IRQ_Handler - interrupt handler, called from the sesnor driver */
void  ICM20948_IRQ_Handler(PIN_Handle handle, PIN_Id pinId) {
  //if(in_advertising_mode == true) {
      Util_startClock(&ICM20948_eventClock);
  //}
}
/* @fn MC6470_ACC_IRQ_Handler - interrupt handler, called from the sesnor driver
void  MC6470_ACC_IRQ_Handler(PIN_Handle handle, PIN_Id pinId) {
  //if(in_advertising_mode == true) {
  //    Util_startClock(&MC6470_ACC_eventClock);
  //}
}
@fn MC6470_MAG_IRQ_Handler - interrupt handler, called from the sesnor driver
void  MC6470_MAG_IRQ_Handler(PIN_Handle handle, PIN_Id pinId) {
  //if(in_advertising_mode == true) {
      Util_startClock(&MC6470_MAG_eventClock);
  //}
}*/
/*********************************************************************
